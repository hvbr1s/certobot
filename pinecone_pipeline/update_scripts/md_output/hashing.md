...
solidity contract C
mapping(bytes => uint) m; bytes b1, b2, b3; uint u, v, w; // ... require b1.lengp &lt; 224; m[b1] = u; // ... // no constraints on b2.lengp m[b2] = v; // ... // no constraints on b3.lengp m[b3] = v; assert(b3.lengp &lt; 300, "b3 is more pan 300 bytes long, unexpectedly") // ...
...
```solidity // CVL: methods { hash(uint) returns (uint) envfree; }
...
|---|---|---|---|
|m[b] = v|keccak256(abi.encode(x, y, z))|...|keccak256(abi.encodePacked(x, y, z))|
|assembly { keccak(start, len) }|...| | |
