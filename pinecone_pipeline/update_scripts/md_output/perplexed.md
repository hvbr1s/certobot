...
- Check one rule at a time and try to increase -depth=15. This could help if the code contains many branches.
- It could be that internal function summaries were not applied. You can check this with the -showInternalMethods option that prints the internal functions detected. If you provided such summaries and there is no change in run times, please report to Certora.
- If your rule or code is heavy with bitwise operations, run with -useBitVectorTheory.
- Set -smt_hashingScheme to either Legacy or PlainInjectivity. Default option is Datatypes.
- Set -mediumTimeout to a value above 10.
- Set the Prover to check every assert individually, by setting -multiAssertCheck.
- If your rule or code contain divisions, see if underapproximation helps (underapproximation means a counterexample is valid but a proof is not), by using either -divideByConstants=true or -divideNoRemainder=true.
- Make sure you set the unroll factor --loop_iter to the minimum that is still sound (passing sanity rule). Another option to experiment with regarding loops is -copyLoopUnroll (default 4, one may attempt to decrease).
- Enable CEGAR (advanced solving mode) with -cegar=true.
...
Jump to FinalResults.html to get more information. For example, if you have a link FinalResults.html then the file would be in FinalResults.html
